...
def my_SIFT():
    origimg = cv2.imread("mountain_1.png")
    img = cv2.cvtColor(origimg, cv2.COLOR_BGR2GRAY)
    keyPoints, discriptors = SIFT(img)

    origimg2 = cv2.imread("mountain_2.png")
    img2 = cv2.cvtColor(origimg2, cv2.COLOR_BGR2GRAY)
    ...
    return drawLines(X1, X2, Y1, Y2, match[0][:, 0], result, num=15)
